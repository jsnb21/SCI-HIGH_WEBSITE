/*:
 * @plugindesc Calls a Loading Screen to make sure JSON files are loaded properly. @author Scott
 *
 * @help
 * Calls a Loading Screen to make sure JSON files are loaded properly.
 */

// File: PreloadJSON.js
(() => {
  const JSON_FILES_TO_LOAD = [
    "quizData.json", // make sure it's in your www/ root if not using "data/"
  ];

  function Scene_LoadingJSON() {
    this.initialize(...arguments);
  }

  Scene_LoadingJSON.prototype = Object.create(Scene_Base.prototype);
  Scene_LoadingJSON.prototype.constructor = Scene_LoadingJSON;

  Scene_LoadingJSON.prototype.initialize = function () {
  ...
  Scene_LoadingJSON.prototype.create = function () {
  ...
  Scene_LoadingJSON.prototype.createLoadingImage = function () {
  ...
  Scene_LoadingJSON.prototype.startLoadingAll = function () {
    ...
    JSON_FILES_TO_LOAD.forEach(filename => {
      ...
      xhr.onerror = () => {
        console.error("Network error loading:", filename);
      };
      xhr.send();
    });
  };

  Scene_LoadingJSON.prototype.update = function () {
  ...
  Scene_LoadingJSON.prototype.startFadeOut = function () {
    this._fadeOutStarted = true;
    this._fadeSprite.opacity = 0;
    this._fadeOutSpeed = 8;
  };

  Scene_LoadingJSON.prototype.updateFade = function () {
    if (this._fadeOutStarted && this._fadeSprite.opacity < 255) {
      this._fadeSprite.opacity += this._fadeOutSpeed;
    }
  };

  const _Scene_LoadingJSON_update = Scene_LoadingJSON.prototype.update;
  Scene_LoadingJSON.prototype.update = function () {
    _Scene_LoadingJSON_update.call(this);
    this.updateFade();
  };

  // Expose the scene to the global scope
  window.Scene_LoadingJSON = Scene_LoadingJSON;
})();
